...
async def main():
    ...
    if all_tasks:
        ...
        df = df.sort_values(by=['Ветка', 'Время'])

        summary = df['Ветка'].value_counts().reset_index()
        summary.columns = ['Название ветки', 'Количество задач']

        with pd.ExcelWriter(file_name) as writer:
            df.to_excel(writer, sheet_name='Список задач', index=False)
            summary.to_excel(writer, sheet_name='Итог за день', index=False)
        
        # Вызываем отправку ПОСЛЕ закрытия writer, чтобы файл успел сохраниться
        send_email(file_name)
        print(f"Готово! Создан {file_name}. Найдено задач: {len(all_tasks)}")
    else:
